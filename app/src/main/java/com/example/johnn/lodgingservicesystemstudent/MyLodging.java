...
public class MyLodging extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_my_lodging);

        getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        pb = new ProgressDialog(MyLodging.this);
        pb.setCanceledOnTouchOutside(false);
        pb.setMessage("Loading...");

        clientId = getIntent().getStringExtra("UserID")+"4";
        ...
        client.setCallback(new MqttCallback() {
            ...
            public void messageArrived(String s, MqttMessage mqttMessage) throws Exception {
                ...
                if (receiverClientId.equals(clientId)) {
                    if (command.equals("004810")) {
                        String size = c.ToString(head[4]);
                        if (size.equals("0")) {
                            Toast.makeText(MyLodging.this, "No recored found", Toast.LENGTH_LONG).show();
                            pb.dismiss();
                        } else {
    ...
    public void SetData(String message) throws Exception {
        ...
        for (int i = 0; i < size; i++) {
            ...
            l.setMessage(c.ToString(body[6]));
            ll.add(l);
        }
        ...
        LodgingAdapter1 lodgingAdapter = new LodgingAdapter1(ll);
        recyclerView.setAdapter(lodgingAdapter);
        pb.dismiss();
        lodgingAdapter.setOnItemClickListener(new LodgingAdapter1.ClickListener() {
            @Override
            public void onItemClick(int position, View v) {
                Intent intent = new Intent(MyLodging.this, ViewMyLodgingDetails.class);
                intent.putExtra("UserID",getIntent().getStringExtra("UserID"));
                intent.putExtra("LodgingID",ll.get(position).getLodgingId());
                intent.putExtra("Message",ll.get(position).getMessage());
